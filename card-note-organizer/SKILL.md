...
将任意形式的原始笔记 — 散乱文字、摘录、灵感碎片、网页内容、文档片段 — 转化为结构化卡片笔记。每张卡片遵循「标题 → 标签 → 总结 → 核心要点 → 洞见 → 图示 → 来源 → 关联笔记」八要素（图示在原文有图时出现，洞见在无值得提炼时可不写，关联笔记在有强相关已有笔记时出现），确保知识可检索、可复用、可关联。
...
2. **标签**（分为三类，混排在同一行，不设子标题：① 出处标签 1-2 个，用 `/` 分割多级来源；② 内容标签 2-5 个，从原文和总结中提炼主题/领域/方法；③ 概念标签，数量不限，标注笔记核心涉及的术语、理论、模型——必须与笔记核心强相关，仅顺带提及的不加）
...
8. **关联笔记**（基于 Reminds 中已有笔记的标题和标签，链接与当前卡片强相关的笔记。格式：`@笔记标题`。写入前调用 `mcp__reminds-mcp__search_notes` 搜索，基于标签语义相关性和主题关联筛选。一次整理多张卡片时，新卡片之间也互相链接）
